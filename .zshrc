######################################################################
#           patricks's zshrc file v0.2.1 , based on:
#		      jdong's zshrc file, v0.1
#
#
######################################################################
...
## Prompt formatting
##
## Displays the current git branch on the right (with a '+' if there are
## uncommitted changes) and change the prompt color to green/red depending
## on the last command's exit value.
##
#####
setopt prompt_subst
check_git_status() {
...
parse_git_branch() {
    ...
    | sed -e '/^  /d' -e 's/* \(.*\)/\1/' )
	if [[ $br == "master" ]] then
		echo "$PR_RED$br$PR_NO_COLOR"
	else
		echo "$PR_LIGHT_YELLOW$br$PR_NO_COLOR"
	fi
}
check_last_status() {
	st=$?
...
PS1='$(check_last_status)%~$PR_NO_COLOR %(!.#.$) '
# PS1="%~$PR_NO_COLOR %(!.#.$) "
...
# Ignore user home directory completion
zstyle ':completion:*:*:*:users' ignored-patterns '*'

# History searching on UP/DOWN key
...
# -G colorizes, -F adds trailing slashes for dirs
alias ls='ls -GF'
alias v=vagrant
...
export PATH="$PATH:/Users/psmith/bin"
